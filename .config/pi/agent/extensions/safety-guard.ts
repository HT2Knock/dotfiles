...
function classifyBash(command: string): Risk | undefined {
  ...
  if (
    /\b(psql|pgcli|mysql|mariadb|sqlite3|sqlcmd|cockroach|sqsh|sqlplus|db2)\b/i.test(
      normalized,
    )
  ) {
    if (
      /\bDROP\s+(?:TABLE|DATABASE|INDEX|SCHEMA|VIEW|MATERIALIZED\s+VIEW|FUNCTION|PROCEDURE|SEQUENCE|TYPE)\b/i.test(
        normalized,
      )
    ) {
      return {
        action: "Drop a database object",
        command,
        reason: "DROP permanently removes database objects and their data.",
        severity: "destructive",
      };
    }
    if (/\bTRUNCATE(?:\s+TABLE)?\b/i.test(normalized)) {
      return {
        action: "Truncate a database table",
        command,
        reason: "TRUNCATE removes all rows without row-level recovery.",
        severity: "destructive",
      };
    }
    if (/\bDELETE\s+FROM\b/i.test(normalized)) {
      return {
        action: "Delete rows from a database table",
        command,
        reason:
          "DELETE FROM removes row data; without a WHERE clause it empties the table.",
        severity: "destructive",
      };
    }
    if (/\bALTER\s+TABLE\b[\s\S]*?\bDROP\b/i.test(normalized)) {
      return {
        action: "Drop a database column or constraint",
        command,
        reason:
          "ALTER TABLE ... DROP permanently removes a column or constraint and its data.",
        severity: "destructive",
      };
    }
  }
...
export default function safetyGuard(pi: ExtensionAPI) {
  ...
  pi.registerCommand("permissions", {
    ...
    handler: async (args, ctx) => {
      ...
      ctx.ui.notify(
        "Usage: /permissions enable | /permissions disable | /permissions status",
        "warning",
      );
    },
  });
